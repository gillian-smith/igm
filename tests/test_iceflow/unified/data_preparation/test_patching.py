...
def create_test_image(height: int, width: int, channels: int = 3) -> tf.Tensor:
    ...
    arr = np.repeat(pixel_ids[:, :, np.newaxis], channels, axis=2).astype(np.float32)
...
class TestOverlapPatching:
    ...
    def test_image_smaller_than_patch_size(self):
        ...
        with the original 250x150 dimensions (not 400x400).
        """
        patcher = OverlapPatching(patch_size=400, overlap=0.25)
    ...
    def test_high_overlap_near_complete(self):
        ...
        """
        patcher = OverlapPatching(patch_size=149, overlap=0.25)
        image = create_test_image(250, 150, channels=3)
    ...
    def test_overlap_approximation_accuracy(self):
        ...
        """
        target_overlap = 0.25
        patcher = OverlapPatching(patch_size=64, overlap=target_overlap)
        ...
        n_patches_y, n_patches_x, stride_y, stride_x, _, _ = (
            patcher._calculate_patching_parameters(
    ...
    def test_various_overlap_values(self):
        ...
        for target_overlap in [0.0, 0.25, 0.5, 0.75]:
            ...
            assert patches.shape[0] > 0, f"Should create patches for overlap={target_overlap}"
    ...
    def test_minimal_padding(self):
        ...
        total_padding = padding_h.numpy() + padding_w.numpy()
    ...
    def test_zero_overlap(self):
        ...
        patcher = OverlapPatching(patch_size=64, overlap=0.0)
        image = create_test_image(256, 256, channels=3)
