...
correspondence_map = {
    "iflo_run_data_assimilation": "iceflow.run_data_assimilation",
    ...
    "data_dir": "pretraining.data_dir",
    "batch_size": "pretraining.batch_size",
    "freq_test": "pretraining.freq_test",
    "train_iceflow_emulator_restart_lr": "pretraining.train_iceflow_emulator_restart_lr",
    "epochs": "pretraining.epochs",
    "min_arrhenius": "pretraining.min_arrhenius",
    "max_arrhenius": "pretraining.max_arrhenius",
    "min_slidingco": "pretraining.min_slidingco",
    "max_slidingco": "pretraining.max_slidingco",
    "min_coarsen": "pretraining.min_coarsen",
    "max_coarsen": "pretraining.max_coarsen",
    "soft_begining": "pretraining.soft_begining",
}
...
omodules = [
    m
    for m in params_dict.pop("modules_postproc", [])
    if m not in ["print_info", "print_comp"]
]
...
# === Generate YAML ===
yaml_structure = {
    "core": {"url_data": nested_params.get("url_data", "")},
    "defaults": [
...
def assign_params(section, module_list, raw_params):
    for mod in module_list:
        if mod == "iceflow":
            iceflow_params = {
                correspondence_map[k]: v
                for k, v in raw_params.items()
                if k in correspondence_map
            }
...
if "optimize" in yaml_structure.get("inputs", {}):
    yaml_structure["processes"]["data_assimilation"] = copy.deepcopy(
        yaml_structure["inputs"]["optimize"]
    )
