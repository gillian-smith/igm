...
def params(parser):
    ...
    parser.add_argument(
        ...
        help="thkobs profiles on which to calculate test score"
    )
    parser.add_argument(
        "--opts_rgi_folder",
        type=str,
...
def initialize(params, state):
    ...
    #nc = xr.open_dataset("optimize.nc")
    #nc = nc.sel(iterations=params.opti_nbitmax)

    rgi_folder = params.opts_rgi_folder
    ...
    if rgi_folder == "": # we use individual RGI outline as downloaded from oggm_shop
        ...
        rgi_folder = "../get_data/"+params.oggm_RGI_ID
    ...
    df = gpd.read_file(params.opts_path_thkobs)
    df.geometry = shapely.force_2d(df.geometry)
    df = df.to_crs(outline.crs)
    
    if params.opts_thkobs_find_method=="outline":
        # spatial join - find data points inside outline
        df = gpd.sjoin(df,outline)
    elif params.opts_thkobs_find_method=="RGIId":
        # find points with same RGI ID as this glacier (even if they are outside the outline)
        df = df[df["RGIId"]==params.oggm_RGI_ID]

    # filter to only points in the test profiles # TODO this is different from how I did it in custom_thkobs - which is better?
    if params.opts_test_profiles: # is not the empty list
        df = df[df["profile_id"].str.strip().str[-1].isin(params.opts_test_profiles)]
    ...
    # TODO does it work with state.x and state.y? if so maybe we can eliminate need to import optimize.nc
    x = state.x.numpy()
    y = state.y.numpy()

    thickness_normalized = df[params.opts_thkobs_column].copy()
    # Rasterize
    gridded = (
    pd.DataFrame(
        {
    ...
    # Mean squared error
    #mse = np.nanmean((nc["thk"].values - thkobs)**2)
    mse = np.nanmean((state.thk - thkobs)**2) # TODO does it work with state.thk

    print("MSE for profiles " + str(params.opts_test_profiles) + f" = {mse}")
