...
def solve_iceflow(cfg: DictConfig, state: State, init: bool = False) -> None:
    ...
    if do_solve:
        ...
        if (not cfg.processes.iceflow.unified.network.pretrained) or cfg.processes.iceflow.unified.network.old_format: # the new pretrained network uses a different normalization scheme
