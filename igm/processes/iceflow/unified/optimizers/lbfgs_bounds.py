...
class OptimizerLBFGSBounds(OptimizerLBFGS):
    ...
    def _force_descent(
        self, p_flat: tf.Tensor, grad_w_flat: tf.Tensor, w_flat: tf.Tensor
    ) -> Tuple[tf.Tensor, Optional[tf.Tensor]]:
        L, U = self.map.get_box_bounds_flat()
        mask = self._get_mask(w_flat, grad_w_flat, L, U)
        p_flat = tf.where(mask, p_flat, tf.zeros_like(p_flat))
        dot_gp = self._dot(grad_w_flat, p_flat)
        return tf.cond(dot_gp >= 0.0, lambda: -grad_w_flat, lambda: p_flat), mask
    ...
    def _apply_step(
        ...
        w_proj = self._project(w_trial, L, U)
        return w_proj, w_trial
    ...
    def _constrain_pair(
        ...
        w_flat = w_prev + s
        proj_changed = tf.not_equal(tf.abs(w_flat - w_trial), 0.0)
    ...
    def _line_search(
        ...
        def eval_fn(alpha: tf.Tensor) -> ValueAndGradient:
            ...
            mask = self._get_mask(w_alpha, grad_flat, L, U)
            p_flat = tf.where(mask, p_flat, tf.zeros_like(p_flat))
    ...
    def minimize_impl(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        w_proj = self._project(w_flat, L, U)
