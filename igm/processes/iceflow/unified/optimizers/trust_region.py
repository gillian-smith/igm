...
#     ) -> tf.Tensor:
#         """
#         Solve trust region subproblem using Steihaug-Tong CG:
...
#             # Skip iteration if already converged or hit boundary
...
#                     if pAp <= 0.0:
#                         if verbose:
#                             tf.print("  NEGATIVE CURVATURE detected at CG iter", i)
...
#                         # Check if we hit the trust region boundary
#                         if tf.norm(x_new) >= delta:
#                             if verbose:
#                                 tf.print("  TRUST REGION BOUNDARY hit at CG iter", i)
...
#             x_norm = tf.norm(x)
#             tf.print("  CG finished | final rel_res =", final_residual, "| ||x|| =", x_norm)
#             if converged:
#                 tf.print("  Converged")
#             if hit_boundary:
#                 tf.print("  Hit boundary")
#             if neg_curvature:
#                 tf.print("  Negative curvature")
...
#                 lambda: tf.minimum(200.0 * new_delta, self.delta_max),
#                 lambda: new_delta
#             )
